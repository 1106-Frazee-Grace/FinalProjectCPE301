...
// I just edited a few things, I'm still working on building the actual circuit to test this all 
...
//setup for LED's (digital pins 22-25, PORTA 0-3)
unsigned char* ddr_a = (unsigned char*) 0x21;
unsigned char* port_a = (unsigned char*) 0x22;

//setup for Button's (Pins 2, 3, 21)
unsigned char* ddr_e = (unsigned char*) 0x2C;
unsigned char* port_e = (unsigned char*) 0x2D;
unsigned char* ddr_c = (unsigned char*) 0x26;
unsigned char* port_c = (unsigned char*) 0x27;

//LED pins are handled via PORTA0-3
const uint8_t START_BUTTON_PIN = 2;
const uint8_t STOP_BUTTON_PIN  = 3;
const uint8_t RESET_BUTTON_PIN = 21;

//setup for Fan (pin 11)
...
//Fan function declarations
void init_fan_pin();
void fan_on(uint8_t pwm = 200);
void fan_off();
...
//LED functions
void init_leds(){
  DDRA |= 0x0F; //set PA0-PA3 as outputs
  PORTA &= ~0x0F; //Turn all LEDs off
}
void led_set_green(bool on){
  if(on){
    PORTA |= (1 << PA0);
...
//Fan Functions: Fan pin configured via DDRB; PWM control handled using analogWrite().
void init_fan_pin(){
  DDRB |= (1 << DDB5);
